...
%% Load data folder
dataFolder = 'experiments/upper-body';
...
allZData = {assistiveZTrajectoryData agnosticZTrajectoryData};
zPlotColors = [colors(3,:);colors(1,:)];
legendOptions = {'Assistive Wrench', 'Agnostic Wrench'};
fileNameSuffixes = ["Helping", "Agnostic"];

%%Time
totalTime = allZData{1,1}.tout;
...
for d = 1:size(allZData,2)
       
    Xdesired = allZData{1,d}.Xdesired.signals.values(startTimeIndex:endTimeIndex,:);
...
for d = 1:size(allZData,2)
    ...
    Xdesired = allZData{1,d}.Xdesired.signals.values(startTimeIndex:endTimeIndex,:);
    Xactual = allZData{1,d}.Xactual.signals.values(startTimeIndex:endTimeIndex,:);
    
    plot(trimTime,Xdesired(:,2)-Xactual(:,2), 'LineWidth',lineWidth,'Color',zPlotColors(d,:));
    hold on;
...
currentFigure = gcf;
title(currentFigure.Children(end), 'Trajectory Tracking Error along Z-axis',...
      'Interpreter', 'latex','FontSize', titleFontSize);
lgd = legend(legendOptions,...
                 'Location','best','Box','off','FontSize',legendFontSize);

save2pdf(fullfile(fullPlotFolder, 'TrajectoryTrackingError-Z.pdf'),fH,300);
...
for d = 1:size(allZData,2)
    ...
    s = allZData{1,d}.s.signals.values(startTimeIndex:endTimeIndex,:);
...
for d = 1:size(allZData,2)
    ...
    sH = subplot(size(allZData,2),1,d); hold on;
    sH.FontSize = fontSize;
    sH.Units = 'normalized';
    
    rHandForces = allZData{1,d}.rhand_wrench.signals.values(startTimeIndex:endTimeIndex,1:3);
...
allYZData = {assistiveYZTrajectoryData agnosticYZTrajectoryData};
yzPlotColors = [colors(2,:);colors(3,:);colors(1,:);colors(1,:)];
legendOptions = {'Assistive Wrench', 'Agnostic Wrench'};
fileNameSuffixes = ["Helping", "Agnostic"];

%%Time
totalTime = allYZData{1,1}.tout;
...
for d = 1:size(allYZData,2)
       
    Xdesired = allYZData{1,d}.Xdesired.signals.values(startTimeIndex:endTimeIndex,:);
...
for d = 1:size(allYZData,2)
    ...
    Xdesired = allYZData{1,d}.Xdesired.signals.values(startTimeIndex:endTimeIndex,:);
    Xactual = allYZData{1,d}.Xactual.signals.values(startTimeIndex:endTimeIndex,:);
    ...
    for i = 1:2
        ...
        plot(trimTime,Xdesired(:,1+i)-Xactual(:,1+i), 'LineWidth',lineWidth,'Color',yzPlotColors(colorCount,:));
        colorCount = colorCount + 1;
        hold on;
...
currentFigure = gcf;
title(currentFigure.Children(end), 'Trajectory Tracking Error along YZ-plane',...
      'Interpreter', 'latex','FontSize', titleFontSize);

save2pdf(fullfile(fullPlotFolder, 'TrajectoryTrackingError-yz.pdf'),fH,300);
...
for d = 1:size(allYZData,2)
    ...
    s = allYZData{1,d}.s.signals.values(startTimeIndex:endTimeIndex,:);
...
for d = 1:size(allYZData,2)
    ...
    sH = subplot(size(allYZData,2),1,d); hold on;
    sH.FontSize = fontSize;
    sH.Units = 'normalized';
    
    rHandForces = allYZData{1,d}.rhand_wrench.signals.values(startTimeIndex:endTimeIndex,1:3);
